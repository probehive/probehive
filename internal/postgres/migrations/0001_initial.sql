...
CREATE TABLE antiforgery_tokens (
    ...
    CONSTRAINT ck_antiforgery_session_hash_length CHECK (octet_length(session_token_hash) = 32),
    CONSTRAINT fk_antiforgery_tokens_sessions FOREIGN KEY (session_token_hash)
        REFERENCES sessions (token_hash) ON DELETE CASCADE
);

CREATE UNIQUE INDEX ux_antiforgery_tokens_session_hash ON antiforgery_tokens (session_token_hash);
