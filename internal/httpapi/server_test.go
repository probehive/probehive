...
func TestSecurityFilterOrderingAndAntiforgeryFailures(t *testing.T) {
	...
	assertProblem(
		t, badOrigin, http.StatusForbidden, originRejectedTitle,
...
func TestConfiguredPublicOriginControlsUnsafeRequests(t *testing.T) {
	...
	assertProblem(
		t, internalOrigin, http.StatusForbidden, originRejectedTitle,
		"The Origin or Referer of this request does not match the expected browser origin.",
	)
...
func TestRevisionNumberRejectsValuesAboveInt32Range(t *testing.T) {
	environment := newTestEnvironment(t, true, 0)
