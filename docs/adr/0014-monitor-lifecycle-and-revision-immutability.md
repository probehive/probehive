...
Check configuration validation lives in `internal/check`, which remains standard-library-only. The consuming feature owns a narrow validator interface that `internal/check` satisfies at composition time, so feature packages do not import sibling implementations or transport and persistence adapters.
...
- The `internal/check` package starts with validation; execution lands behind the same Check Type and schema version identity later.
