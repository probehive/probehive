...
Use `/api/v1` as the canonical first public HTTP contract and publish one reviewed OpenAPI document for each supported API major version. `/api` may be a gateway routing prefix, but it is not a separate unversioned API contract and must not silently alter compatibility semantics.
...
Browser sessions use host-only `Secure`, `HttpOnly`, `SameSite=Lax` cookies carrying opaque random tokens with bounded lifetime and rotation after authentication or privilege changes. Store only token hashes in PostgreSQL, and bind synchronizer antiforgery state to the server-side session so restarts and multiple API replicas share one security state. Browser access and refresh tokens are never stored in local storage or session storage.
...
- Authentication remains valid across API restarts and replicas when they share the PostgreSQL session store.
